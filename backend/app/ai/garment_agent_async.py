        ...
        async with self.page_semaphore:
            ...
            if not img_matches:
                return None
                    ...
                    # Filter out non-product images (like size charts, logos, icons)
            filtered_imgs = [
                url for url in img_matches
                if not any(bad in url.lower() for bad in ["size", "chart", "logo", "icon", "guide", "banner", "placeholder"])
            ]

            if not filtered_imgs:
                print("⚠️ No valid product images found on:", href)
                return None
            
            # Try the first valid image
            img_url = filtered_imgs[0]
            print(f"🖼️ Analyzing image: {img_url}")
